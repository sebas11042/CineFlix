...
CREATE OR ALTER PROCEDURE ocupar_asientos_funcion
    @idFuncion INT,
    @asientos NVARCHAR(MAX) -- formato: '12,13,14'
AS
BEGIN
    SET NOCOUNT ON;

    -- Convertir la cadena CSV a XML para parsearla
    DECLARE @xml XML = CAST('<root><id>' + 
        REPLACE(@asientos, ',', '</id><id>') + 
        '</id></root>' AS XML);

    -- Actualizar cada asiento en AsientoFuncion
    UPDATE af
    SET af.ocupado = 1
    FROM AsientoFuncion af
    INNER JOIN (
        ...
        FROM @xml.nodes('/root/id') AS t(x)
    ) AS parsed ON af.id_asiento = parsed.id_asiento
